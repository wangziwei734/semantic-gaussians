...
def build_vlpart(checkpoint=None):
    ...
    cfg.merge_from_list(
        [
            ...
            ["p2", "p3", "p4", "p5"],
            "MODEL.ROI_BOX_CASCADE_HEAD.IOUS",
            [0.5, 0.6, 0.7],
...
class VLPart(nn.Module):
    def __init__(
        ...
        self.text_embed_cache = None

        self.register_buffer("pixel_mean", torch.tensor([123.675, 116.280, 103.530]).view(-1, 1, 1), False)
        self.register_buffer("pixel_std", torch.tensor([58.395, 57.120, 57.375]).view(-1, 1, 1), False)
    ...
    def inference(
        ...
        if self.text_embed_cache is None:
            self.text_embed_cache = self.get_text_embeddings(text_prompt)
        results, _ = self.roi_heads(images, features, proposals, self.text_embed_cache)
        if do_postprocess:
...
def custom_detector_postprocess(
    results: Instances, output_height: int, output_width: int, max_shape, mask_threshold: float = 0.5
):
    """
