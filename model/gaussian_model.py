...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int):
        ...
        self._features_semantic = torch.empty(0)
    ...
    def requires_grad_(self, requires: bool):
        ...
        self._features_dc.requires_grad_(requires)
        self._features_rest.requires_grad_(requires)
        self._scaling.requires_grad_(requires)
        self._rotation.requires_grad_(requires)
        self._opacity.requires_grad_(requires)
        self._features_semantic.requires_grad_(requires)
        self._times.requires_grad_(requires)

    def get_covariance(self, scaling_modifier=1):
        return self.covariance_activation(self.get_scaling, scaling_modifier, self._rotation)

    def get_covariance_rotation(self, scaling_modifier=1, world_rotate=None):
        L = build_scaling_rotation(scaling_modifier * self.get_scaling, self._rotation)
        actual_covariance = world_rotate.transpose(0, 1) @ L @ L.transpose(1, 2) @ world_rotate
        symm = strip_symmetric(actual_covariance)
        return symm
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, random_init=False):
        ...
        dist2 = torch.clamp_min(
            distCUDA2(torch.from_numpy(np.asarray(pcd.points).copy()).float().cuda()),
            0.0000001,
        )
    ...
    def create_semantic(self, num_channels=768):
        self._features_semantic = nn.Parameter(
            torch.zeros((self._xyz.shape[0], num_channels), dtype=torch.float, device="cuda").requires_grad_(True)
        )
        self._times = nn.Parameter(
            torch.zeros((self._xyz.shape[0], 1), dtype=torch.float, device="cuda").requires_grad_(True)
        )
    ...
    def load_dynamic_npz(self, path, t):
        if self.dynamic_npz is None:
            ...
            is_fg = self.dynamic_npz["seg_colors"][:, 0] > 0.5
            self.is_fg = is_fg

            opacities = self.dynamic_npz["logit_opacities"]
            scales = self.dynamic_npz["log_scales"]
            features_extra = np.zeros((opacities.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1), dtype=np.float32)
            self._features_rest = nn.Parameter(
                torch.tensor(features_extra, dtype=torch.float, device="cuda")
                .transpose(1, 2)
                .contiguous()
    ...
    def get_locs_and_features(self, feature_type="all"):
        ...
        features_rest = self._features_rest.detach().clone().cpu().numpy()
